...
def step3():
    ...
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="git clone git://github.com/SchedMD/slurm.git")
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="cd slurm ; sudo make ; sudo make install")
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="sudo cp /usr/share/doc/slurm-client/examples/slurm.conf.simple.gz /home/pi/slurm")
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="sudo gzip -d /home/pi/slurm/slurm.conf.simple.gz")
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="sudo mv /home/pi/slurm/slurm.conf.simple /home/pi/slurm/slurm.conf")
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command=f'''sudo sed -i 's/SlurmctldHost=workstation/SlurmctldHost={manager}({trueIP})/g' /home/pi/slurm/slurm.conf''')
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command=f'''sudo sed -i "$(( $(wc -l </home/pi/slurm/slurm.conf)-2+1 )),$ d" /home/pi/slurm/slurm.conf''')
    print(Printer.write(results))
    ...
    for x in range(len(hostnames)):
        results = Host.ssh(hosts=manager,
                           command=f'''echo "NodeName={hostnames[x]} NodeAddr={trueIPs[x]} CPUs={coreCounts[x]} State=UNKNOWN" | sudo tee /home/pi/slurm/slurm.conf -a''')
        print(Printer.write(results))
    results = Host.ssh(hosts=manager, command=f'''echo "PartitionName=mycluster Nodes={workers} Default=YES MaxTime=INFINITE State=UP" | sudo tee /home/pi/slurm/slurm.conf -a''')
    print(Printer.write(results))
    ...
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="sudo cp ~/cgroup.conf /home/pi/slurm/cgroup.conf")
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="sudo cp ~/cgroup_allowed_devices_file.conf /home/pi/slurm/cgroup_allowed_devices_file.conf")
    print(Printer.write(results))
    ...
    print(Printer.write(results))
    results = Host.ssh(hosts=manager, command="sudo cp /home/pi/slurm/slurm.conf /home/pi/slurm/cgroup.conf /home/pi/slurm/cgroup_allowed_devices_file.conf /clusterfs")
    print(Printer.write(results))
...
def step4():
    ...
    print(Printer.write(results))
    results = Host.ssh(hosts=workers, command="cd slurm ; sudo make ; sudo make install")
    print(Printer.write(results))
    results = Host.ssh(hosts=workers, command="sudo cp /clusterfs/munge.key /etc/munge/munge.key")
    print(Printer.write(results))
    results = Host.ssh(hosts=workers, command="sudo cp /clusterfs/slurm.conf /home/pi/slurm/slurm.conf")
    print(Printer.write(results))
    results = Host.ssh(hosts=workers, command="sudo cp /clusterfs/cgroup* /home/pi/slurm/")
    print(Printer.write(results))
